...
def update_daily_activity(name, activity, response, filename_date_range=None, keep=False):
	...
	if len(df) >0:
		if df[_eq_day_name[activity]].iloc[-1] == df2[_eq_day_name[activity]].iloc[0]:
			...
			if df2[_eq_activity_list[activity]].iloc[0] > df[_eq_activity_list[activity]].iloc[-1]:
...
_eq_entry_list = {"dailyCalories": 'activities/heart', "dailySteps": 'activities/tracker/steps', "fitbitWearTimeViaHR": 'activities/heart', "minuteCalories": 'activities/heart'}
...
if __name__ == "__main__":
	...
	parser.add_argument('--debug', action='store_true', help="read data from test.json instead of getting online.")
	...
	if args.debug:
		...
		while date <= end_date:
			res = authed_client.intraday_time_series(_eq_entry_list["minuteCalories"], base_date=date.strftime("%Y-%m-%d"))
			print(res['activities-heart-intraday'])
	...
	#print(authed_client.time_series(, period="max"))